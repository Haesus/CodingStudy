...
struct SheetToggleHostView: View {
    let store: StoreOf<SheetToggleFeature>

    var body: some View {
        WithViewStore(store, observe: { $0 }) { vs in
            VStack(spacing: 8) {
                Button("시트 열기") { vs.send(.open) }
            }
            .sheet(isPresented: vs.binding(\.$isPresented)) {
                VStack(spacing: 16) {
                    Text("시트 내용입니다").font(.headline)
                    Button("닫기") { vs.send(.close) }
                }
                .padding()
                .detentsIfAvailable()
            }
